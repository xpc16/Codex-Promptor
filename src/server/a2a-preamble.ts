...
export function buildA2aPreamble(input: PreambleInput): string {
  ...
  const lines = [
    ...
    "",
    "## 协作节奏",
    "要让另一个对话做事，唯一的办法是上面这个命令。它的队列、历史和最终回答都不要去读写 data/ 下的文件，那是存储不是接口，直接改会和正在运行的后端冲突。",
    "第一步先 list 拿到目标的 tabId，再 send。send 只是把消息放进对方队列，不会在本轮等到回复；派发后请结束本轮，对方的反馈会作为一条新 prompt 进入本对话的队列。不要轮询等待。",
