...
第 2 条是这套架构的必然结果。**开了 E2EE、用一句他不知道的口令，这件事就做不到**——他仍能打开页面，但只会看到一个要密钥的框，服务端发过去的是密文。
...
之后远端打开页面会先弹「需要加密密钥」，把口令输进去即可。**对一下指纹**，应该和本机一模一样。输错会当场告诉你「密钥不正确」。密钥只存在那个浏览器里，换设备要重新输。
...
- 它加密的是**内容**。元数据（何时连、传了多少、多频繁）和按键节奏仍然可见；你自己硬盘上 `data/tabs/` 里的提示词和回答是明文。
