...
需要 **Node.js 22～24**（`node --version` 确认，25 及以上不行）和 git。CLI 一个都不装也能用，「终端」类型的对话只是个 PowerShell。
...
流量路径是 **你的电脑 → Cloudflare → 你的手机**。中间那段内容对两方可见：

1. **Cloudflare** —— TLS 在它那里终止。
2. **域名所有者（你朋友）** —— 他控制 Access 策略。**他随时可以把自己的邮箱加进放行名单，打开这个页面，看到你的终端画面、对话内容和最终回答，你不会收到任何提示。**
...
本机打开 `http://127.0.0.1:4317/` → 新建对话 → 右下角下拉框选**「端到端加密」** → 「加密密钥」框里打一句只有你知道的话（**中文可以**，或点「生成」拿随机的）→ 「确定并打开」。

标签会自动改名成 `E2EE`，并显示一个**密钥指纹**，形如 `K7M2-9QXF`。

之后远端打开页面会先弹「需要加密密钥」，把口令输进去即可。**对一下指纹**，应该和本机一模一样。输错会当场告诉你「密钥不正确」，不会让你用着用着才发现。密钥只存在那个浏览器里，换设备要重新输。
...
- E2EE 依赖浏览器的 WebCrypto，**只在 `https://` 或 `http://127.0.0.1` 下可用**。走子域名没问题；用局域网 IP（`http://192.168.x.x`）直连则加密不工作。
- 它加密的是**内容**。元数据（何时连、传了多少、多频繁）和按键节奏仍然可见；你自己硬盘上 `data/tabs/` 里的提示词和回答也是明文——那是 BitLocker 的活。
