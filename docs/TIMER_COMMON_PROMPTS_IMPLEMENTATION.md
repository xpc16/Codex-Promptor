...
- 编辑发生在页面内存 draft，保存时整份 Timer 一次提交；
- 关闭编辑器或切换 Timer 不弹出放弃确认；按 tab 与 Timer id 缓存未保存 draft，页面刷新时自然清空，不写磁盘也不增加网络请求；
...
- 间隔：正数（保存两位小数，换算后四舍五入到整分钟）+ 小时/天 + 宿主机当地时间锚点 + 可选当地结束时间；每轮在名义计划时间后的 0～3 分钟内稳定随机提交，重启不会重新抽取。

三种规则都以 Promptor 宿主机的当地时间为准。间隔按当地日历推进：每天一次会在跨 DST 后继续保持相同钟点；每小时一次也按当地钟面小时推进。浏览器只提交 `datetime-local` 字符串，不做 UTC 换算，避免远端浏览器把手机或另一台电脑的时区带入调度。
...
第一版不提供任意时区选择。一次、每周与间隔规则都跟随 Promptor 宿主机时区：
...
- interval 的锚点和可选结束时间保存为无时区的当地日期时间，并按当地日历小时/天推进；旧版 UTC 锚点在服务启动时兼容读取并一次性规范化为宿主机当地时间。
